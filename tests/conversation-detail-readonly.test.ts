...
const { ConversationDetail } = loadModule("src/components/v2/inbox/conversation-detail.tsx") as {
  ConversationDetail: (p: Record<string, unknown>) => React.ReactElement;
};
...
test("conversation detail hides the reply composer when read-only", () => {
  ...
  assert.match(editable, /reply-composer/);
  assert.doesNotMatch(readonly, /reply-composer/);
  // The thread itself still renders in read-only (visibility for coaching).
  assert.match(readonly, /Ana/);
});
  ...
  function load(absolutePath: string): Record<string, unknown> {
    ...
    runInNewContext(output, {
      ...
      require(id: string) {
        ...
        if (id === "@/components/v2/inbox/reply-composer") {
          return {
            ReplyComposer: () => React.createElement("div", { "data-component": "reply-composer" }),
          };
        }
        if (id === "next/link") {
          return {
