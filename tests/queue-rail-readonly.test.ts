...
const { QueueRail } = loadModule("src/components/v2/inbox/queue-rail.tsx") as {
  QueueRail: (p: Record<string, unknown>) => React.ReactElement;
};
...
test("queue rail is selection-only: read-only is presentation-neutral", () => {
  ...
  // Renders the rail in both modes.
  assert.match(editable, /data-component="queue-rail"/);
  assert.match(readonly, /data-component="queue-rail"/);
  // No mutation controls exist in the rail, so the markup is identical.
  assert.equal(editable, readonly);
  assert.doesNotMatch(readonly, /data-action=/);
});
