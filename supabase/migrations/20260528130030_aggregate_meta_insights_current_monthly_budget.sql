-- Make aggregate_meta_daily_insights monthly_budget a current active budget.
--
-- monthly_budget is a current active daily budget, monthlyized once per
-- distinct live ad set in the result group using the month of p_end. It is not
-- historical budget exposure across every month/day in the requested range.
--
...
as $$
    ...
    select
      ...
      row_number() over (
        partition by
          ...
          ad_set_id
        order by
          case when delivery_status = 'live' then 0 else 1 end,
          date_start asc
      ) as budget_rank
    from filtered
  ),
  grouped as (
    select
      ...
      round(sum(case when budget_rank = 1 and delivery_status = 'live' and daily_budget > 0 then daily_budget * days_in_month else 0 end), 2) as monthly_budget,
