-- Add delivery_status filter support to aggregate_meta_daily_insights.
--
-- The Optimize page status picker is a page-level data filter. The RPC now
-- maps current Meta ad/ad-set/campaign status into the UI vocabulary used by
-- the filter bar: live, paused, off.
    ...
    select
      ...
      case
        ...
        when upper(coalesce(a.effective_status, a.status, s.effective_status, s.status, c.effective_status, c.status, '')) = 'PAUSED'
          then 'paused'
        when coalesce(a.effective_status, a.status, s.effective_status, s.status, c.effective_status, c.status, '') = ''
          then 'unknown'
        else 'off'
