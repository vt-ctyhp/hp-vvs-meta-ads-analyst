...
as $$
  ...
  filtered as (
    select
      ...
      e.daily_budget_entity_key,
      e.lifetime_budget_entity_key,
      e.budget_remaining_entity_key,
  ...
  grouped as (
    select
      ...
      round(sum(case when monthly_budget_rank = 1 and daily_budget > 0 then daily_budget * days_in_month else 0 end), 2) as monthly_budget,
      round(sum(case when daily_budget_rank = 1 and daily_budget > 0 then daily_budget else 0 end), 2) as daily_budget,
      round(sum(case when lifetime_budget_rank = 1 and lifetime_budget > 0 then lifetime_budget else 0 end), 2) as lifetime_budget,
      round(sum(case when budget_remaining_rank = 1 and budget_remaining > 0 then budget_remaining else 0 end), 2) as budget_remaining,
  ...
  order by
    ...
    case when lower(coalesce(p_sort_direction, 'desc')) = 'asc' and p_sort_field = 'daily_budget' then daily_budget end asc nulls last,
    case when lower(coalesce(p_sort_direction, 'desc')) = 'desc' and p_sort_field = 'daily_budget' then daily_budget end desc nulls last,
    case when lower(coalesce(p_sort_direction, 'desc')) = 'asc' and p_sort_field = 'lifetime_budget' then lifetime_budget end asc nulls last,
    case when lower(coalesce(p_sort_direction, 'desc')) = 'desc' and p_sort_field = 'lifetime_budget' then lifetime_budget end desc nulls last,
    case when lower(coalesce(p_sort_direction, 'desc')) = 'asc' and p_sort_field = 'budget_remaining' then budget_remaining end asc nulls last,
    case when lower(coalesce(p_sort_direction, 'desc')) = 'desc' and p_sort_field = 'budget_remaining' then budget_remaining end desc nulls last,
